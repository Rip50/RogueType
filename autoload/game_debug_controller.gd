...
var zombie := preload("res://characters/enemies/zombie.tscn")

func _ready() -> void:
	pass


func _find_scene(scene_name: String) -> Object:
	var object := get_tree().root.find_child(scene_name, true, false)
	if object == null:
		push_warning("Object with name %s cannot be found in node tree" %scene_name)
	return object
